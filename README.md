...
Defaults for new sites: `--space` = your personal space · `--name` = file/folder name slugified · `--visibility` = `unlisted` (`private` · `members` · `team` also available). Replacing a site without `--visibility` preserves its current tier. Point at another instance with `POSTPLAN_API_URL=https://… postplan <cmd>`.
