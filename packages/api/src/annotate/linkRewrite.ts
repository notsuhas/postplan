...
//
// Bug this fixes: content.ts only injects this client when the request carries ?postplan_annotate=1
// (content.ts:160). Relative links inside an uploaded page don't carry it, so every in-iframe
// navigation after the first page loads WITHOUT the client — no postplan:ready postMessage, so (a)
 ...
 *  `postplan_annotate=1`, and return the resolved absolute URL with the param set — or null to leave
