...
/** Resolve the site and authorize the caller, or hand back the response to return. Starring adds NO
 *  rule of its own: if `checkAccess` lets you open a page you may star it, including your own
 *  private pages and private pages shared with you — a star is a private, per-user bookmark, so
 *  there is nothing for a visibility tier to protect. 404/403/410 precedence is inherited. */
async function resolveStarTarget(c: Context<AppEnv>): Promise<ResolvedSite | Response> {
...
// lost access to simply vanish from the feed while its row waits for a flip back.
...
stars.post('/:space/:site/star', async (c) => {
  const site = await resolveStarTarget(c)
