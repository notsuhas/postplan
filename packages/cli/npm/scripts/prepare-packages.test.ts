...
import { preparePackages } from './prepare-packages'
...
describe('preparePackages', () => {
  test('stages all native binaries and synchronizes package versions', async () => {
    ...
    await preparePackages('1.2.0', distDir, npmDir)
  ...
  test('rejects versions that would move the existing npm package backward', async () => {
    await expect(preparePackages('1.1.0', '/unused', '/unused')).rejects.toThrow('must be at least 1.2.0')
  })
})
