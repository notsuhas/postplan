...
describe('parseIntent', () => {
  ...
  test('parseintent-rejects-bad-shape', () => {
    expect(parseIntent(ev({ data: { type: 'postplan:unknown' } }), expected)).toBeNull()
    expect(parseIntent(ev({ data: 'not-an-object' }), expected)).toBeNull()
    expect(parseIntent(ev({ data: { type: 'postplan:select' } }), expected)).toBeNull() // missing quote
    expect(parseIntent(ev({ data: { type: 'postplan:select', quote: '' } }), expected)).toBeNull() // empty quote
  })
  ...
  test('parseintent-truncates-long-select-quote', () => {
    ...
    const res = parseIntent(ev({ data: { type: 'postplan:select', quote: 'y'.repeat(9000) } }), expected)
  ...
  test('parses a select-clear intent', () => {
    expect(parseIntent(ev({ data: { type: 'postplan:select-clear' } }), expected)).toEqual({ type: 'clear' })
  })

  test('parses the dismissal intents the parent cannot observe inside the iframe', () => {
    expect(parseIntent(ev({ data: { type: 'postplan:click-away' } }), expected)).toEqual({ type: 'clickAway' })
    expect(parseIntent(ev({ data: { type: 'postplan:escape' } }), expected)).toEqual({ type: 'escape' })
  })

  test('parses the comment-key intent (#117) — payload-free, like the dismissals', () => {
    expect(parseIntent(ev({ data: { type: 'postplan:comment-key' } }), expected)).toEqual({ type: 'commentKey' })
  })

  test('parses the ask-key intent — payload-free, mirrors comment-key', () => {
    expect(parseIntent(ev({ data: { type: 'postplan:ask-key' } }), expected)).toEqual({ type: 'askKey' })
  })
  ...
  test('dismissal intents are rejected on a wrong origin or wrong source, like every other intent', () => {
    for (const type of ['postplan:click-away', 'postplan:escape', 'postplan:comment-key', 'postplan:ask-key']) {
  ...
  test('a ready handshake with an over-cap filePath is rejected, not truncated', () => {
    ...
    expect(parseIntent(ev({ data: { type: 'postplan:ready', filePath: 'x'.repeat(2001) } }), expected)).toBeNull()
  })
})
