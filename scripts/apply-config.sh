...
python3 - "$APP_URL" "$CONTENT_URL" "$SUPERADMIN_EMAIL" "${ADMIN_EMAILS:-}" "$D1_DATABASE_ID" "$KV_NAMESPACE_ID" <<'PY'
import re, sys
...
def render(example, real, host):
    ...
    s = re.sub(r'"ADMIN_EMAILS": "[^"]*"',     f'"ADMIN_EMAILS": "{admins}"', s)
    s = re.sub(r'"database_id": "[^"]*"',      f'"database_id": "{d1}"', s)
    s = re.sub(r'("binding": "GLANCE_SESSIONS", "id": ")[^"]*"', rf'\g<1>{kv}"', s)
...
s = re.sub(r'https://glance-content[A-Za-z0-9.\-]*', content, s)
...
echo "  admins   $SUPERADMIN_EMAIL${ADMIN_EMAILS:+, $ADMIN_EMAILS}"
